...
'''Python wrapper for the CGrid library'''
...
cdef class Cellgrid(object):
    ...
    def __init__(self, cellcutoff.Cell cell, double spacing=1):
        """Initialize a Cellgrid object.

        Parameters
        ----------
        cell
            Describes periodic boundary conditions
        spacing
            The desired distance between neighbouring faces of the subcells
            in which grid points are sorted.
        """
        pass

    def __dealloc__(self):
        """Deallocate the cellgrid."""
        if self._this != NULL:
            del self._this
    ...
    property cell:
        def __get__(self):
            """Cell describing the periodic boundary conditions."""
    ...
    property subcell:
        def __get__(self):
            """Subcell used for sorting grid points."""
            cdef np.ndarray[double, ndim=2] vecs = np.zeros((3, 3), float)
    ...
    property shape:
        def __get__(self):
            """Repetitions of subcell along each cell vector to obtain periodic cell."""
    ...
    property grid_array:
        def __get__(self):
            """A numpy.ndarray view on the low-level grid definition."""
    ...
    property points:
        def __get__(self):
            """A numpy.ndarray view on the Cartesian grid points."""
            return self.grid_array['cart']

    property weights:
        def __get__(self):
            """A numpy.ndarray view on the Quadrature weights."""
            return self.grid_array['weight']

    property indices:
        def __get__(self):
            """A numpy.ndarray view on the grid point indices.

            These define the order of the points prior to sorting.
            """
    ...
    def append_many(self, np.ndarray[double, ndim=2] points not None,
        ...
        """Add many grid points to the grid."""
    ...
    def sort(self):
        """Sort the grid points into subcells."""
        self._this.sort()
    ...
    def integrate_cutoff(self, np.ndarray[double, ndim=1] center not None,
        ...
        -------
        integral
            The numerical integral within the cutoff sphere.
    ...
    def integrate(self, *factors):
        ...
        ----------
        *factors
            Multiple arrays of grid values, which will be multiplied elementwise
            before integrating.
